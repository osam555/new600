...
def initialize_session_state():
    ...
    default_settings = {
        ...
        'second_lang': 'english',
        'third_lang': 'chinese',
        'first_repeat': 1,    # 0도 선택 가능하도록 수정
        'second_repeat': 1,   # 0도 선택 가능하도록 수정
        'third_repeat': 1,    # 0도 선택 가능하도록 수정
        'eng_voice': 'Steffan',
        'kor_voice': '선희',
        'zh_voice': 'Yunjian',
        'jp_voice': 'Nanami',
...
def create_settings_ui(return_to_learning=False):
    ...
    if return_to_learning:
        ...
        with col1:
            ...
            settings['first_repeat'] = st.selectbox("1순위 재생(횟수)",
                                   options=[0, 1, 2, 3],
        ...
        with col2:
            ...
            settings['second_repeat'] = st.selectbox("2순위 재생(횟수)",
                                    options=[0, 1, 2, 3],
        ...
        with col3:
            ...
            settings['third_repeat'] = st.selectbox("3순위 재생(횟수)",
                                   options=[0, 1, 2, 3],
...
async def start_learning():
    ...
    while True:
        for i, (eng, kor, chn, jpn, vn) in enumerate(zip(english, korean, chinese, japanese, vietnamese)):
            ...
            # 재생 횟수가 0이 아닌 언어만 설정에 추가
            for lang, repeat in lang_settings:
                if repeat > 0:  # 재생 횟수가 0이면 건너뛰기
                    lang_configs.append({
